...
# 女娲量骨·结构尺(固化版):检查一个本地路径或 GitHub 仓库的发布就绪度。
# 基础检查源自 skill-miniloop(已并入女娲作为标配体检尺),
...
if [[ -f "$README" ]] && grep -q 'npx skills add' "$README" 2>/dev/null; then
  pass "README 含一行接引 (npx skills add)"
else
  warn "README 缺 npx skills add 一行接引"
fi

if [[ -f "$README" ]] && grep -q 'skills\.sh/b/' "$README" 2>/dev/null; then
  pass "README 含 skills.sh 接引计数徽章"
else
